...
- 生成 `.avante/verification-report.md` 审查报告
...
**路径**：`.avante/context-summary-[任务名].md`
...
□ 已查阅上下文摘要文件：.avante/context-summary-[任务名].md
□ 将使用以下可复用组件：
  - [组件1]: [路径] - [用途]
  - [组件2]: [路径] - [用途]
□ 将遵循命名约定：[具体说明]
...
所有任务执行产生的工作文件必须写入项目本地 `.avante/` 目录（而非全局 `~/.avante/`）：
##### ```
<project>/.avante/
    ├── context-summary-[任务名].md   ← 上下文摘要（输出）
    ├── operations-log.md             ← 决策和操作记录（输出）
    └── verification-report.md        ← 验证报告（输出）
##### ```
...
- 上下文收集：
  1. **强制检索清单**（7步，编码前必做）
  2. **充分性验证**（7项检查，必须全部通过）
  3. **生成上下文摘要**（`.avante/context-summary-[任务名].md`）
...
- 生成评分和建议（写入 `.avante/verification-report.md`）
- 根据评分决策：
  - ≥90分 → 通过
  - <80分 → 退回修改
  - 80-89分 → 仔细审阅后决策
...
#### 步骤1：结构化快速扫描（必须）
执行框架式收集，记录到 `.avante/context-summary-[任务名].md`：
- 位置：功能在哪个模块/文件？
...
- 聚焦单个疑问，不发散
- 提供代码片段证据，而非猜测
- 更新 `.avante/context-summary-[任务名].md`
...
- ❌ 上下文文件写入错误路径（必须是 `.avante/` 而非 `~/.avante/`）
